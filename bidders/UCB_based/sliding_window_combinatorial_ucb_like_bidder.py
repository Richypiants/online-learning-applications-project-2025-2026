...
class SlidingWindowCombinatorialUCBLikeBidder(CombinatorialUCBLikeBidder):
    ...
    def __init__(self, B, T, valuations, environment, window_size=None):
        ...
        # default window size choice: tau = O(sqrt(T log T)), since it should be divided by the # changes or by T if unknown
        self.window_size = int(window_size if window_size is not None else min(T, np.ceil(2 * np.sqrt(np.log(T)))))      # theoretical choice: tau = O(sqrt(T log T)/#changes), with #changes = T if unknown
    ...
    def _confidence_bounds(self):
        ...
        # NOTE: some values here might be NaN (when cache is empty for that arm), but the np.where in the next step puts them back to 0.0 to avoid issues in the LP

        mask = N_pulls_last_W != 0
        radius = np.broadcast_to(self.range[:, None], N_pulls_last_W.shape).astype(float).copy()      # per-campaign range broadcast over arms; TODO: reflect changes of range in base_ucb, or find a way to make them the same
