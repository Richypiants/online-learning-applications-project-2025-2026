...
class BaseUCBLikeBidder(Bidder):
    def __init__(self, B, T, valuations, environment: Environment):
        ...
        # TODO: think about estimating the win_probabilities instead and then computing these from the standard formulas
        #self.avg_f = np.array([self.valuations[0] - self.bids for _ in range(self.N_CAMPAIGNS)]) / 2        # ORIGINAL: np.zeros(self.K)
        self.avg_f = np.array([np.zeros(self.K) for _ in range(self.N_CAMPAIGNS)])                      # ALTERNATIVE: as above
        self.avg_c = np.array([np.zeros(self.K) for _ in range(self.N_CAMPAIGNS)])                      # ALTERNATIVE: self.bids / 2
    ...
    def _confidence_bounds(self):
        #TODO: actually, the range represents the width of the domain of the random variable, so it'd be better to have a custom range for each bid and f/c
    ...
    def learn(self, f_t, c_t, m_t=None):
        # TODO: use numpy indexing instead, just like in the primal-dual bidder
        ...
        self.B -= np.sum(c_t)
        self.t += 1
        # TODO: also update RHO so that the agent can adapt to the remaining budget? But then this would be unfair for 
        # the clairvoyant which should be updated in the same way too...